...
namespace Hamburger_Application.Validations.Dessert
{
    public class UpdateDessertVMValidator:AbstractValidator<DessertUpdateVM>
    {
        ...
        {
            RuleFor(f => f.Name).NotEmpty().MaximumLength(30).WithMessage("Dessert name must not be empty & maximum characters must be 30");
			...
			RuleFor(f => f.Piece).GreaterThan(0).WithMessage("Priece cannot be a negative value");
		}
